...
|----------|---------|-------------|
| `SANDBOX_NAME` | `sandbox` | Name used for the Docker container, compose project, and CLI commands (e.g. `openharness shell $SANDBOX_NAME`) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `HOST_SSH_DIR` | `~/.ssh` | Path to your host SSH directory, mounted read-only into the container (with `ssh.yml` overlay). Lets git inside the sandbox authenticate using your existing SSH keys. |
